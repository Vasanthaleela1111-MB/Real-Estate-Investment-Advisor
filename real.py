...
data=pd.read_csv("C:\AIML\Projects\cleaned_data.csv")
df=pd.read_csv("C:\AIML\Projects\Feature_data.csv")
...
""")
  ...
  if selected == "1. What is the distribution of property prices?":
    ...
    ax.bar(price.index,price.values)
    ax.set_xlabel("Furnished_Status")
    ax.set_ylabel("Price_per_SqFt")
    ...
    fig, ax = plt.subplots()
    price=df.groupby('Total_Amenities')['Price_per_SqFt'].count()
    plt.hist(price.index,price.values)
    st.pyplot(fig)
