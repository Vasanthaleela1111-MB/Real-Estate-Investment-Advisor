...
classification = pickle.load(open(os.path.join(base_path,'classification.pkl'),'rb'))
scale = pickle.load(open(os.path.join(base_path,'classification_scaler.pkl'),'rb'))
regression = pickle.load(open(os.path.join(base_path,'model.pkl'),'rb'))
...
""")
  ...
  if selected == "1. What is the distribution of property prices?":
    ...
    if st.button("Predict"):
        ...
        sample["State"] = encode_value("State", state)
        sample["City"] = encode_value("City", city)
        sample["Locality"] = encode_value("Locality", locality)
        sample["Parking_Space"] = encode_value("Parking_Space", parking)
        sample["Facing"] = encode_value("Facing", facing)
        sample["Security"] = encode_value("Security", security)
        sample["Public_Transport_Accessibility"] = encode_value("Public_Transport_Accessibility", transport)
        ...
        # scale input
        sample = sample[scale.feature_names_in_]
        prediction = classification.predict(sample)
        probability = classification.predict_proba(sample)
    ...
    price_sqft = st.number_input("Price per SqFt",key='price')
    size = st.number_input("Size in SqFt",key='size')

    amenity_density = st.number_input("Amenity Density Score")

    transport = st.selectbox(
        "Public Transport Accessibility",
        ["High","Medium","Low"],key='transport'
    )
    ...
    property_type = st.selectbox(
        "Property Type",
        ["Apartment","Villa","Independent House"],key='property'
    )
    ...
    hospitals = st.number_input("Nearby Hospitals",key='hospitals')
    year = st.number_input("Year Built",key='year')

    # amenities = st.number_input("Amenities")

    if st.button("Predict Future Price"):

        sample = pd.DataFrame(columns=regression.feature_names_in_)
        sample.loc[0] = 0
        ...
        # Column order
        sample = sample.reindex(columns=regression.feature_names_in_, fill_value=0)

        # Prediction
        price = regression.predict(sample)[0]
